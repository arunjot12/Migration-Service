...
export async function verifyMigration(
    ...
    for (const [ [_sourceKey, sourceData], [destKey, destData]] of zipFork) {
        ...
        } else if (destKey === xxhashAsHex("Contracts", 128) + xxhashAsHex("CodeStorage", 128).slice(2)) {
            let failed = await verifyContractsCodeStorage(sourceData, source.api, destData, destination.api);
            if (failed.length > 0) {
                failedVerification.push(...failed);
            }
        }
...
async function verifyContractsCodeStorage(
    oldData: Array<[StorageKey, Uint8Array]>,
    oldApi: ApiPromise,
    newData: Array<[StorageKey, Uint8Array]>,
    newApi: ApiPromise
    ...
    for(let [key, value] of oldData) {
        ...
        let oldAccount = oldApi.createType('PrefabWasmModule', value);

        let newScale = newDataMap.get(key.toHex());
        if (newScale !== undefined) {
            console.log("*****************newScale ================");
            let newAccount = newApi.createType('PrefabWasmModule', newScale);
...
async function prepareContract(
    ...
    // Match against the actual storage items of a pallet.
    for(let [palletStorageItemKey, values] of Array.from(keyValues)) {
        if (palletStorageItemKey === (xxhashAsHex("Contracts", 128) + xxhashAsHex("CodeStorage", 128).slice(2))) {
            xts.set(palletStorageItemKey, await prepareContractsCodeStorage(toApi, values));
        } else if (palletStorageItemKey === (xxhashAsHex("Contracts", 128) + xxhashAsHex("ContractInfoOf", 128).slice(2))) {
            xts.set(palletStorageItemKey, await prepareContractsContractInfoOf(toApi, values));
        } else {
            return Promise.reject("Fetched data that can not be migrated. PatriciaKey is: " + palletStorageItemKey);
        }
    }
    ...
    for (const item of values) {
        ...
        if (item instanceof StorageMapValue) {
            if (packetOfAccounts.length === maxAccounts - 1  || counter === values.length) {
                ...
                const abc = toApi.tx.migration.migrateSystemAccount(packetOfAccounts);
                console.log("abc ============> ", abc);
...
async function prepareContractsCodeStorage(
    toApi: ApiPromise,
    values: StorageItem[]
): Promise<Array<SubmittableExtrinsic<ApiTypes, SubmittableResult>>> {
    let xts: Array<SubmittableExtrinsic<ApiTypes, SubmittableResult>> = new Array();
    ...
    for (const item of values) {
        ...
        if (item instanceof StorageMapValue) {
            if (packetOfAccounts.length === maxAccounts - 1  || counter === values.length) {
                // push the last element and prepare extrinsic
                packetOfAccounts.push(await retrieveIdAndAccount(item))
                const abc = toApi.tx.migration.migrateContractCodeStorage(packetOfAccounts);
                console.log("abc ============> ", abc);
                xts.push(abc)
    ...
    for (const item of values) {
        ...
        if (item instanceof StorageMapValue) {
            if (packetOfAccounts.length === maxAccounts - 1  || counter === values.length) {
                // push the last element and prepare extrinsic
                packetOfAccounts.push(await retrieveIdAndAccount(item))
                xts.push(toApi.tx.migration.migrateContractContractInfoOf(packetOfAccounts))
